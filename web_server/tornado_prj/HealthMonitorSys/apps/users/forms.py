...
MOBILE_REGEX = "^1[345789]\d{9}$|^1[48]7\d{8}$|^176\d{8}$"  # 手机正则表达式
...
class DeviceNoForm(Form):
    device_no = StringField("设备序列号", validators=[DataRequired(message="请输正确入设备序列号")])


class DoctorIdForm(Form):
    doctor_id = IntegerField("医护人员ID", validators=[DataRequired(message="请输正确入医护人员ID")])
