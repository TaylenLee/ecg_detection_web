...
from apps.admin.forms import RegisterForm, LoginForm, ChangePasswordForm
...
class LoginHandler(RedisHandler):
    async def post(self, *args, **kwargs):
        ...
        if form.validate():
            ...
            try:
                user = await self.application.objects.get(Admin, mobile=mobile)
    ...
    async def post(self, *args, **kwargs):
        ...
        if register_form.validate():
            ...
            if not self.redis_conn.get(redis_key):
                ...
                # 验证用户是否存在
                try:
                    existed_users = await self.application.objects.get(Admin, mobile=mobile)
                    self.set_status(400)
                    re_data["mobile"] = "用户已经存在"
                except Admin.DoesNotExist as e:
                    user = await self.application.objects.create(Admin, mobile=mobile, nick_name="admin", password=password)
                    re_data["id"] = user.id
        else:
            self.set_status(400)
